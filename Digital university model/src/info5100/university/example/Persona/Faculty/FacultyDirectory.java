...
public class FacultyDirectory {
  ...
  public FacultyProfile newFacultyProfile(Person p, Department dept) {
    FacultyProfile sp = new FacultyProfile(p, dept);
    teacherlist.add(sp);
    return sp;
}
...
public FacultyProfile findByPerson(Person p) {
    for (FacultyProfile fp : teacherlist) {
        if (fp.isMatch(p.getPersonId())) {
            return fp;
        }
    }
    return null;
